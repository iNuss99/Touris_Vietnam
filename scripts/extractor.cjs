const fs=require('fs');
let c=fs.readFileSync('frontend/src/i18n/translations.js','utf8');
c=c.replace('export default translations;','module.exports = translations;');
fs.writeFileSync('scripts/temp_extractor.cjs',c);
...
fs.writeFileSync('docs/DuLieuTour_Botpress.txt',out);
fs.unlinkSync('scripts/temp_extractor.cjs');
console.log('Done!');
